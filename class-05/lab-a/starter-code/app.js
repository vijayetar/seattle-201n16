...
function sumAndMultiply(a, b, c) { //eslint-disable-line
  ...
  var add = firstSum[0];
  var secondSum = sum(add,c);
  var add2 = secondSum[0];
  console.log(add2);

  var firstProduct = multiply(a,b);
  var product = firstProduct[0];
  var secondProduct = multiply(product,c);
  var product2 = secondProduct[0];
  console.log(product2);

  var sentOne = (a + ' and ' + b + ' and ' + c + " sum to " + add2 + '.');
  console.log(sentOne);
...
function sumArray(sumArr) { //eslint-disable-line
  ...
  for (var i = 0; i < sumArr.length; i++) {
    add = sum(add,sumArr[i]);
    add = add[0];
    console.log(add);
      if (i === (sumArr.length - 1)) {
        var numbers = numbers + sumArr[i];
      }
      else {
        var numbers = numbers + sumArr[i] + ',';
      }  
...
function multiplyAnyArray(dynamicArray) { //eslint-disable-line
  ...
  for (var i = 0; i < dynamicArray.length; i++) {
    mult = multiply(mult,dynamicArray[i]);
    mult = mult[0];
    console.log(mult);
      if (i === (dynamicArray.length - 1)) {
        var numbers = numbers + dynamicArray[i];
      }
      else {
        var numbers = numbers + dynamicArray[i] + ',';
      }  
